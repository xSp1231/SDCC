...
from .CASC import PrototypicalCalibrationBlock
...
def inference_on_dataset(model, data_loader, evaluator, cfg=None):
    ...
    if cfg.TEST.CASC_ENABLE:
        logger.info("Start initializing CASC module, please wait a seconds...")
        CASC = CASC(cfg)
        print("启动CASC")
        print("启动CASC")
        print("启动CASC")
    ...
    with inference_context(model), torch.no_grad():
        for idx, inputs in enumerate(data_loader):
            ...
            if cfg.TEST.CASC_ENABLE:
                outputs = CASC.execute_calibration(inputs, outputs)
